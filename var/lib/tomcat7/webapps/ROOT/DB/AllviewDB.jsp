...
<%	
	...
	ResultSet rset = stmt.executeQuery("select name, studentid, kor, eng, mat, (kor+eng+mat), ((kor+eng+mat)/3) from examtable;");
...
<%
	int to = 9;
		while (rset.next()){
			...
			out.println("<td width=50><p align=center>" + Integer.toString(rset.getDouble(7))+"</p></td>");
			out.println("</tr>");
			}
		}
	rset.close();
...
if (startpoint != 1) {			//1페이지가 포함되었을땐 <<, <가 나타나지 않는다
	out.println("<a href=AllviewDB.jsp?pageNum="+1+">&lt;&lt;</font></a>");	//맨 앞으로
	out.println("<a href=AllviewDB.jsp?pageNum="+(startpoint-10)+">&lt;</font></a>");		//10페이지 앞으로
}
for(int i=0; i<10; i++){		//10개 페이지 링크 출력, 페이지번호와 시작 번호를 매칭...수열
	if((10*startpoint-9)+(i*10) > cnt) break;		//총량을 벗어날 경우 페이지 링크를 더 표시하지 않는다
	out.println("<a href='AllviewDB.jsp?pageNum="+startpoint+"&from="+((10*startpoint-9)+(i*10))+"&cnt=9' target='main'>"+(startpoint+i)+"</a>");
}
if (10*startpoint < cnt-100) {		//마지막 페이지에서는 >, >>가 안나타나도록
	out.println("<a href=AllviewDB.jsp?pageNum="+(startpoint+10)+">&gt;</font></a>");		//10페이지 뒤로
	out.println("<a href=AllviewDB.jsp?pageNum="+(cnt/100*10)+">&gt;&gt;</font></a></center><br>");	//마지막 페이지로
}
%>
</body>
</html>