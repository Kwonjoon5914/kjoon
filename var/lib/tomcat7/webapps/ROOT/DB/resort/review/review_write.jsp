...
<%	
	...
	if(datgeullev==null){
	ps = conn.prepareStatement("insert into review (title, date, content, file) values (?,?,?,?);");
	...
	stmt.executeUpdate("update review set rootid = last_insert_id() where id = last_insert_id();");

	}else{
		
	stmt.executeUpdate("update review set recnt=recnt+1 where rootid= " + onegeul + " and recnt >= " + datgeulorder + ";");
	ps = conn.prepareStatement("insert into review (title, date, content ,rootid, relevel, recnt, file) values (?,?,?,?,?,?,?);");
	ps.setString(1, title);
	ps.setString(2, date2);
	ps.setString(3, content);
